...
// --- FUNÇÃO AUXILIAR PARA EXECUTAR SCRIPTS PYTHON (VERSÃO CORRIGIDA) ---
function executePythonScript(scriptName, args = []) {
  // A mudança está aqui: Usamos path.join para criar um caminho absoluto e seguro
...
app.get('/api/me', isAuthenticated, (req, res) => {
  // Se a requisição chegou até aqui, o usuário está autenticado.
  // O middleware 'isAuthenticated' já validou req.session.user.
  // Apenas retornamos os dados do usuário que estão na sessão.
  res.status(200).json({
    success: true,
...
// Rota para fazer logout e destruir a sessão
app.post('/api/logout', (req, res) => {
  req.session.destroy((err) => {
    if (err) {
      // Se houver um erro ao destruir a sessão, informe.
...
app.post('/api/service-orders/:orderId/cipa-analysis', async (req, res) => {
    ...
    `;
        ...
        // NOVO: Lógica de envio de e-mail condicional
        try {
            const [orderData] = await connection.execute('SELECT OSSM_ID FROM service_orders WHERE id = ?', [orderId]);
            const ossmId = orderData[0]?.OSSM_ID;
...
app.get('/api/daily-tasks', async (req, res) => {
    ...
    `;
        ...
        // NOVO: Envio de e-mail para o PCM
        try {
            const [orderData] = await connection.execute('SELECT OSSM_ID FROM service_orders WHERE id = ?', [orderId]);
            const ossmId = orderData[0]?.OSSM_ID;
    ...
    `;
        ...
        try {
            ...
            if (ossmId && labAnalysis.length > 0) {
                ...
                // Caso não precise de análise de laboratório no fluxo, vai direto para o solicitante
                 executePythonScript('pcmtorequester.py', [ossmId, requesterEmail]);
            }
        } catch (emailError) {
        ...
        `;
            ...
            case 'submit_pcm_execution':
                ...
                await connection.execute(
                    'INSERT INTO pcm_execution (service_order_id, execution_description, executed_by_email, execution_date, houve_solicitacao_compra, numero_solicitacao_compra) VALUES (?, ?, ?, ?, ?, ?)',
                    [
