...
def send_notification(recipient_emails, ossm_id):
    subject = f"Reavaliação Laboratorial Requerida para a OS: #{ossm_id}"
    message_body = f"Olá, Equipe do Laboratório,<br><br>O serviço referente à Ordem de Serviço (<b>OS: {ossm_id}</b>) foi executado e agora requer sua reavaliação para finalização."
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Uso: python notify_lab_for_reevaluation.py <ossm_id> <recipient_emails>", file=sys.stderr)
        sys.exit(1)
    ossm_id = sys.argv[1]
    emails = sys.argv[2]
