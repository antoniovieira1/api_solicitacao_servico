...
def send_notification(recipient_email, ossm_id):
    subject = f"Sua Solicitação de Serviço foi Finalizada: OS #{ossm_id}"
    message_body = f"Olá,<br><br>Sua solicitação de serviço, agora registrada como Ordem de Serviço (<b>OS: {ossm_id}</b>), foi concluída com sucesso."
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Uso: python notify_requester_on_completion.py <ossm_id> <recipient_email>", file=sys.stderr)
        sys.exit(1)
    ossm_id = sys.argv[1]
    email = sys.argv[2]
