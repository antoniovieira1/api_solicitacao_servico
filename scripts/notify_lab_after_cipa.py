...
def send_notification(recipient_emails, ossm_id):
    subject = f"Análise Laboratorial Requerida para a OS: #{ossm_id}"
    message_body = f"Olá, Equipe do Laboratório,<br><br>A Ordem de Serviço (<b>OS: {ossm_id}</b>) foi aprovada pela CIPA/Segurança e agora requer sua avaliação."
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Uso: python notify_lab_after_cipa.py <ossm_id> <recipient_emails>", file=sys.stderr)
        sys.exit(1)
    ossm_id = sys.argv[1]
    emails = sys.argv[2]
