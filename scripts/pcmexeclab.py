...
def send_notification(recipient_email, ossm_id):
    subject = f"OSSM {ossm_id}: Reavaliação Laboratorial Necessária"
    message_body = f"Olá, Equipe do Laboratório,<br><br>O serviço de manutenção da <b>OSSM {ossm_id}</b> foi executado e agora o item precisa de uma reavaliação para garantir sua conformidade."
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Uso: python pcmexeclab.py <ossm_id> <recipient_email>", file=sys.stderr)
        sys.exit(1)
